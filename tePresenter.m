...
classdef tePresenter < handle
    ...
    properties
        ...
        EchoLog = false
        FullDebugOutput = false
        DisableOpenGLErrorChecking = false
        ClearCommandWindowDuringListExecution = true
        MapMaximumDisplayHeight = 25;
        DisableKeyboardDuringSession = true
        SilentMode = false
        DisableKeyboard = false
        StampVideoOnSessionStart = true      
        DisableTextDrawing = false;
        EnableWindowCopy = false;
        % et calibration
        ETCalibPoint
        ETPosthocCalibrationEnabled = false
        ETPosthocCalibrationFrequencySecs = 300
        ETCalibStartSize = 8
        ETCalibEndSize = .25
        ETCalibDurWait = 1.2
        ...
        % light patch
        LightPatchEnabled = false
        LightPatchPositionPreset = 'bottomleft'
    ...
    properties (SetAccess = private)
        ...
        Paused = false
        % eye tracking
        ETPosthocCalibrationResults
        ETCalibrationResults
        TaskLayerPtr
        CurrentDrawing
...
%         PosixTimeOffset
    ...
    properties (Dependent)
        ...
        MonitorSize
        WindowMode
        BorderlessWindowPosition
        BorderlessPreviewPosition
        MonoScreenMode
        DrawingSize
        DrawingAspectRatio
        SkipSyncTests
...
%         CameraResolutionY        
    ...
    properties (Access = private)
        ...
        prMonitorNumber
        prMonitorSize
        prWindowMode = 'fullscreen'
        prBorderlessWindowPosition
        prBorderlessPreviewPosition
        prBorderlessDimensions
        ...
        prPreviewScale = .2
        prPreviewScaledFontSize = 16;
        prDrawPaneOpen = false
        prPreviewDrawMode = 'composite'
...
%         prCameraTextureTimestamp = nan
        ...
        prETFaceAngle = 0
        prDisableStimulusDrawing = false
        prDoSchematic = false
        prDoComposite = true
        % light patch
...
%         prLogBufferIdx = 1
        ...
        prETCalibStatus
        prCalibDebug
        prETPHCLastOffset = teGetSecs
    end
    
    properties (Constant)
        % general
        CONST_VERSION_NO = 0.91
        ...
        KB_TASK_FORWARD         =   '=+'
        % alpha blending
        CONST_ALPHA_BLEND_FACTOR_SRC = 'GL_SRC_ALPHA';
%         CONST_ALPHA_BLEND_FACTOR_SRC = 'GL_ONE';
        CONST_ALPHA_BLEND_FACTOR_DEST = 'GL_ONE_MINUS_SRC_ALPHA';        
    end

    properties (Hidden)
        SkipAllFixations = false;
    end
...
%             obj.UpdatePosixOffset;
        ...
        % init PTB graphics
            ...
            % set verbosity to none (unless FullDebugOutput is set, 
            % otherwise set it max). Store the current value in
            % prPrevVerbosityFlag so we can restore it at shutdown. 
        ...
        % init eye tracking calibraiton point
...
%             end
        ...
        % currently in memory
            ...
            if ~isempty(obj.Tracker.prPreviousSessionEyeTracking)
                et = obj.Tracker.prPreviousSessionEyeTracking;
                if isa(et, 'uint8')
                    et = getArrayFromByteStream(et);
                end
                if isa(et.eyetracker, 'uint8')
                    et.eyetracker = getArrayFromByteStream(et.eyetracker);
                end
                obj.EyeTracker.ReceiveGazeFromResumedSession(...
                    et.eyetracker.Buffer);
            end
        ...
        % layers 
        function InitialiseLayers(obj)
            
%             obj.prLayers = teCollection('hstruct');
            obj.prLayers = teLayers;
            
            % get size of monitor and drawing pane, in pixels
            [dw, dh] = teWidthHeightFromRect([0, 0, obj.Resolution]);
            switch obj.WindowMode
                case {'fullscreen', 'monoscreenmode'}
                    [mw, mh] = teWidthHeightFromRect(Screen('Rect', obj.MonitorNumber));
                case 'borderless'
                    mh = dh;
                    mw = dw;
            end
            ...
            %                 name              w   h   draw on window                  draw on preview             % z-pos     % bgcol             % position              % schematic-choice        
            obj.CreateLayer('preview',          mw, mh, obj.DrawPreviewOnMainWindow,    true,                       [],         [],                 [],                     'preview')
            obj.CreateLayer('task_details',     mw, mh, false,                          true,                       [],         [],                 [],                     'preview')              
            obj.CreateLayer('et_face',          fw, fh, obj.DrawETFaceOnMainWindow,     false,                      [],         [],                 [],                     'preview')
            obj.CreateLayer('et_calib_point',   mw, mh, false,                          false,                      [],         [],                 [],                     'main')
            obj.CreateLayer('et_aois',          dw, dh, obj.DrawGazeOnMainWindow,       obj.DrawAOIsOnPreview,      [],         [],                 [],                     'preview')
            obj.CreateLayer('et_gaze',          mw, mh, obj.DrawGazeOnMainWindow,       obj.DrawGazeOnPreview,      [],         [],                 [],                     'preview')
            obj.CreateLayer('et_calib',         mw, mh, obj.DrawCalibOnMainWindow,      obj.DrawCalibOnPreview,     [],         [],                 [],                     'preview')
            obj.CreateLayer('timing',           tw, th, false,                          obj.DrawTimingOnPreview,    [],         [],                 'bottomright',          'preview')
            obj.CreateLayer('light_patch',      mw, mh, obj.LightPatchEnabled,          obj.LightPatchEnabled,      [],         [],                 [],                     'main')
            obj.CreateLayer('screenflash',      mw, mh, false,                          false,                      [],         [],                 [],                     'main')          
            obj.CreateLayer('logo',             mw, mh, true,                           true,                       [],         obj.COL_TE_PURPLE,  [],                     'main')          
            obj.CreateLayer('tasks',            dw, dh, true,                           true,                       [],         [],                 [],                     'main')
            obj.CreateLayer('previewschema',    mw, mh, false,                          true,                       [],         [],                 [],                     'preview')

            % store texture ptr of the 'tasks' layer 
            obj.TaskLayerPtr = obj.prLayers('tasks').ptr;
            
        end
        
        function CreateLayer(obj, name, w, h, drawOnWindow,...
            drawOnPreview, z, bgcol, posPreset, schematicChoice)
            
            if ~obj.prWindowOpen
                error('A window must be open to create a layer.')
            end
            
            % defaults
            
                % default width and height is full screen
                res_monitor = Screen('Rect', obj.MonitorNumber);
                ...
                % by default, draw to both main window and preview
                if ~exist('drawOnWindow', 'var') || isempty(drawOnWindow)
                    drawOnWindow = true;
                end
                if ~exist('drawOnPreview', 'var') || isempty(drawOnPreview)
                    drawOnWindow = true;
                end

                % default z pos is on top of last layer
                if ~exist('z', 'var') || isempty(z)
                    z = obj.prLayerCurrentZ;
                    obj.prLayerCurrentZ = obj.prLayerCurrentZ + 1;
                else
                    % if z has been specified, and it is the same as currentz,
                    % increment currentz so we don't overwrite this layer next
                    % time one is created
                    if z == obj.prLayerCurrentZ
                        obj.prLayerCurrentZ = z + 1;
                    end
                end

                % default BG colour is black/transparent
                if ~exist('bgcol', 'var') || isempty(bgcol)
                    bgcol = [0, 0, 0, 0];
                end

                % default position is centred
                if ~exist('posPreset', 'var') || isempty(posPreset)
                    posPreset = 'centre';
                end
                
                % schematic choice is empty
                if ~exist('schematicChoice', 'var')
                    schematicChoice = [];
                end
            ...
            s.w = w;
            s.h = h;
            s.drawOnWindow = drawOnWindow;
            s.drawOnPreview = drawOnPreview;
            s.z = z;
            s.posPreset = posPreset;
            s.schematicChoice = schematicChoice;
            
            % if using schematic PreviewDrawMode, then any textures
            % (offscreen windows) are attached to the preview window, not
            % the main window
            if isequal(obj.PreviewDrawMode, 'schematic') &&...
                    isequal(schematicChoice, 'preview')
                contextPtr = obj.prPreviewPtr;
            else
                contextPtr = obj.prWindowPtr;
            end
            
            % create PTB offscreen window
            s.ptr = Screen('OpenOffscreenWindow',...
                contextPtr, bgcol, [0, 0, w, h], [], [], obj.CONST_MULTISAMPLE);
             
            % enable alpha blending for texture and set font
            Screen('BlendFunction', s.ptr, obj.CONST_ALPHA_BLEND_FACTOR_SRC,...
                obj.CONST_ALPHA_BLEND_FACTOR_DEST);         
            
            % add
            try
                obj.prLayers(name) = s;
            catch ERR
                % don't need this PTB offscreen window now, so close it
                Screen('Close', s.ptr)
                switch ERR.message
                    case 'Keys must be unique.'
                        error('Layer name %s already exists - cannot create a new layer with this name.',...
                            name)
                    otherwise
                        rethrow(ERR)
                end
        ...
        function ClearLayer(obj, varargin)
            num = length(varargin);
            cellfun(@(x) obj.ClearTexture(obj.prLayers(x).ptr),...
                varargin);
%             obj.ClearTexture(obj.prLayers(name).ptr);
        end
        
        function UpdateLayerVisibility(obj, layer, location, val)
            if isempty(obj.prLayers), return, end
            res = obj.prLayers(layer);
            if isempty(res)
                error('No layer named %s found.', layer)
            end
            switch location
                case 'mainWindow'
                    fld = 'drawOnWindow';
                case 'preview'
                    fld = 'drawOnPreview';
                otherwise 
                    error('Location must be ''mainWindow'' or ''preview''.')
            end
            obj.prLayers(layer).(fld) = val;
        end
...
%                 fprintf('Prepared stim %s\n', stim.Filename);
            ...
            if immediate
                ...
                Screen('DrawTexture', obj.ptr, stim.TexturePtr,...
                    [], rectPx, angle, [], alpha);
                % if using schematic preview drawing, also draw to preview
                if obj.prDoSchematic &&...
                        ~isempty(stim.PreviewTexturePtr)
                    Screen('DrawTexture', obj.prLayers('preview'),...
                        stim.PreviewTexturePtr, [], rectPx, angle, [], alpha);
                    % todo - support drawing schematics to preview in
                    % immediate mode
                end
            else 
                % add to draw buffer
                ...
                obj.prDrawBuffer(obj.prDrawBufferIdx, :) =...
                    [stim.TexturePtr, rectPx, angle, alpha, stim.PreviewTexturePtr];
        ...
        function [flipTime, flipTimeLocal] = RefreshDisplay(obj, varargin)
            ...
            % housekeeping
                ...
                doCalibPointUpdate = ~ismember('noCalibPointUpdate', varargin);
                goFast = ismember('gofast', lower(varargin));
            ...
            % draw buffer and flip screen
            
                obj.Draw
                obj.CompositeMainWindow
                tm(2) = teGetSecs * 1000;
...
%                 Screen('Flip', obj.prPreviewPtr);
            ...
            if obj.prDoSchematic
%                 Screen('FillRect', obj.prPreviewPtr);
                obj.ClearLayer('preview')
            end
            
            % clear main window and preview
            
                if obj.EnableWindowCopy
                    % copy current screen contents to texture 
                    Screen('CopyWindow', obj.ptr, obj.CurrentDrawing);
                end
        ...
        % stimulus drawing
            ...
            if obj.prDrawBufferIdx > 1
                ...
                buf_stim = obj.prDrawBufferStim(1:obj.prDrawBufferIdx - 1, :);
                % todo - detect and avoid impossible rect values (or should
                % we catch them when the buffer is added to?)
                if obj.EnableFastDrawBuffer
                
                    % reshape
                    texPtr      = buf(:, 1)';
                    texPtrPrev  = buf(:, 8');
                    rect_preview= buf(:, 2:5)';
                    angle       = buf(:, 6)';
                    alpha       = buf(:, 7)';
                    % draw
                    Screen('DrawTextures', obj.ptr, texPtr, [], rect_preview,...
                        angle, [], alpha);
                    
                    % if using schematic preview drawing, also draw to
                    % preview layer
                    if obj.prDoSchematic && ~all(isempty(texPtrPrev)) &&...
                            all(texPtrPrev ~= 0)
                        Screen('DrawTextures', obj.prLayers('preview').ptr,...
                            texPtrPrev, [], rect_preview, angle, [], alpha);
                    end
                    
                    % draw schematic representations of stimuli to the
                    % preview window
                    obj.DrawStimSchematicsToPreview(buf, buf_stim);
                    
                else                 
                
                    % draw textures in a serial loop
                    numTextures = obj.prDrawBufferIdx - 1;
                    for t = 1:numTextures
                        
                        Screen('DrawTexture', obj.ptr, buf(t, 1), [],...
                            buf(t, 2:5), buf(t, 6), [], buf(t, 7));  
                        
                        if isSchematic
                            texPtr_preview =...
                                obj.prDrawBufferStim{t}.PreviewTexturePtr;
                            if ~isempty(texPtr_preview)
                                Screen('DrawTexture', obj.prLayers('preview').ptr,...
                                    texPtr_preview, [], buf(t, 2:5), buf(t, 6),...
                                    [], buf(t, 7));      
                                obj.DrawStimSchematicsToPreview(...
                                    buf(t, :), obj.prDrawBufferStim{t});
                            end
                        end
...
%             % draw calib stimlus, if one is active
%             if ~isempty(obj.ETCalibPoint) && obj.ETCalibPoint.Active
%                 obj.DrawCalibPoint
%             end
%             
...
%             end
            ...
            rect_dp_inner = Screen('Rect', obj.ptr);
            rect_dp_outer = Screen('Rect', obj.prWindowPtr);
            rect_dp = round(teCentreRect(rect_dp_inner, rect_dp_outer));
            Screen('DrawTexture', obj.prWindowPtr, obj.ptr, [], rect_dp) 
            
            % tell PTB that no more drawing will happen to the main window
            Screen('DrawingFinished', obj.prWindowPtr);     
            
%             else
%                 Screen('DrawTexture', obj.prWindowPtr, obj.prLayers('et_calib').ptr);
%             end
            
%             % optionally draw preview to main window
%             if obj.DrawPreviewOnMainWindow 
%                 Screen('DrawTexture', obj.prWindowPtr, obj.prLayers('preview').ptr);
%             end
%             
%             % optionally draw et face to main window
%             if obj.DrawETFaceOnMainWindow
%                 Screen('DrawTexture', obj.prWindowPtr, obj.prLayers('et_face').ptr);
%             end            
%             
...
%                 Screen('DrawTexture', obj.prWindowPtr, obj.prLayers('et_gaze').ptr);
%             end           
%             
  
%             
%             % clear overlays
%             obj.ClearLayer('et_face', 'et_gaze', 'timing');
...
%             Screen('DrawTexture', obj.prPreviewPtr, obj.prLayers('preview').ptr);         
%             
%             if obj.prDisableStimulusDrawing
%                 % copy calibration point texture to the main and preview 
%                 % windows
%                 Screen('DrawTexture', obj.prPreviewPtr, obj.prLayers('et_calib_point').ptr);
%             end            
%             
% %             rect_inner = [0, 0, obj.Resolution]; 
% %             rect_outer = Screen('Rect', obj.prPreviewPtr);
% %             rect_preview = teCentreRect(rect_inner, rect_outer);
...
%             % window
%             Screen('FillRect', obj.prLayers('preview').ptr, obj.BackColour);
        ...
        function DrawAOIs(obj)
            ...
            % any AOIs with prop == NaN must be replaces with zeros
            idx_nan = isnan(aoi_prop);
            aoi_prop(idx_nan) = ones(sum(idx_nan), 1);
            ...
            % draw AOIs
            Screen('FrameRect', obj.prLayers('et_aois').ptr,... % outine
                [col_dk, 225], aoi_frame, 5)
            Screen('FillRect', obj.prLayers('et_aois').ptr,...  % fill
                [col_lt, 100], aoi_fill)
...
%             if Screen('AsyncFlipCheckEnd', obj.prWindowPtr) ~= 0                
                for a = 1:numAOIs
                    ...
                    % draw text
                    if ~obj.DisableTextDrawing
                        Screen('DrawText', obj.prLayers('et_aois').ptr, aoi_str,...
...
%             end
        ...
        function DrawGaze(obj)
            ...
            % clear previous frame from layers
            obj.ClearLayer('et_gaze')
            obj.ClearLayer('et_face')
            
            % get monitor and drawing pane resolution, calculate offset of
            % drawing pane to monitor edge (window limits)
            res_drawing = obj.Resolution;
            res_monitor = Screen('Rect', obj.prWindowPtr);
            offset_drawing = round((res_monitor(3:4) - res_drawing) / 2);
            
            % draw gaze
            
                % get the latest n samples of gaze
                t2 = teGetSecs;
                t1 = t2 - obj.prEyeTrackerPreviewGazeHistory;
                gaze = obj.EyeTracker.GetGaze(t1, t2);
                if isempty(gaze)
                    if obj.FullDebugOutput
                        teEcho('[tePresenter.DrawGaze]: No gaze data returned on this refresh.')
                    end
                    return
                end
                numSamps = size(gaze, 1);
                
                % drawn gaze is faded out backwards in time. Calculate
                % alpha values for each gaze sample            
                alphaInc = 255 / (numSamps - 1);            % alpha increment per sample
                a_range = (0:alphaInc:255)';                % vector of alpha values
                a_range(end) = 255;                         % ensure latest sample is fully opaque
                if length(a_range) > numSamps               % if we got this wrong, truncate to gaze length
                    a_range = a_range(1:numSamps);              
                end                    
                
                % get eye validity, and validity across eyes (either left
                % or right is valid)
                val_l = gaze(:, 4);
                val_r = gaze(:, 19);
                val = val_l | val_r; 

                % now preprocess the gaze, transform it to PTB coords, and
                % draw to the layer
                if any(val)
                    ...
                    gx_l = round(gx_l * res_drawing(1)) + offset_drawing(1);
                    gy_l = round(gy_l * res_drawing(2)) + offset_drawing(2);
                    gx_r = round(gx_r * res_drawing(1)) + offset_drawing(1);
                    gy_r = round(gy_r * res_drawing(2)) + offset_drawing(2);   
                    ...
                    % draw
                    Screen('DrawDots', obj.prLayers('et_gaze').ptr, dots, radius,...
                        col', [], 1);   
        ...
        % then optionally display a message about this. 
            ...
            if gazeOffscreen && obj.DrawGazeOffscreenMessage
                ...
                % set font size
                oldFontSize = Screen('TextSize', obj.prLayers('et_gaze').ptr, 64);
                
                % get bounds of text box
                tb = Screen('TextBounds', obj.prLayers('et_gaze').ptr, msg, 0, 0);
                
                % centre text box in texture
                tb = teCentreRect(tb, res_monitor);
%                 tb = teCentreRect(tb, [0, 0, obj.Resolution]);
                
                % draw red background
                rect_redbg_inner = Screen('Rect', obj.ptr);
                rect_redbg_outer = Screen('Rect', obj.prWindowPtr);
                rect_redbg = teCentreRect(rect_redbg_inner, rect_redbg_outer);
                Screen('FillRect', obj.prLayers('et_gaze').ptr,...
                    [200, 000, 000, 180], rect_redbg);
                
                % draw text, shadow first, then overlay
                if ~obj.DisableTextDrawing
                    Screen('DrawText', obj.prLayers('et_gaze').ptr, msg, tb(1) + 1, tb(2) + 1,...
                        [000, 000, 000, 255], [000, 000, 000, 000]);
                    Screen('DrawText', obj.prLayers('et_gaze').ptr, msg, tb(1), tb(2),...
                        obj.COL_LABEL_FG, [000, 000, 000, 000]);
                end
                
                % draw frame, again with a shadow
                rect_frame = tb + [-20, -20, 20, 20];
                Screen('FrameRect', obj.prLayers('et_gaze').ptr,...
                    [000, 000, 000], rect_frame + 1, 3);
                Screen('FrameRect', obj.prLayers('et_gaze').ptr,...
                    obj.COL_LABEL_FG, rect_frame, 3);
                
                % restore font size
                Screen('TextSize', obj.prLayers('et_gaze').ptr, oldFontSize);
            ...
            rect_main = Screen('Rect', obj.prLayers('et_face').ptr);
%             rect_main = [0, 0, obj.ScalePoint(obj.DrawingSize, 'cm2px')];
            ...
            f_rect_win_ol = round([...
                ...
                y_px_win_ol + ((f_h_px_win / z_ol) / 2)+ (f_h_px_win / 9)]);                                                                   
            % draw et face
            Screen('DrawTexture', obj.prLayers('et_face').ptr, f_tex_win,...
                [], f_rect_win, angle, [], f_alpha_win);
            % draw et face outline
            Screen('DrawTexture', obj.prLayers('et_face').ptr,...
        ...
        function DrawTaskDetails(obj)
            ...
            % clear texture
            obj.ClearLayer('task_details')
            
            % set font size
            prevTextSize = Screen('TextSize', obj.prLayers('task_details').ptr, 50);
            ...
            % get size of text
            bounds = Screen('TextBounds', obj.prLayers('task_details').ptr, str);
            tw = bounds(3);
            th = bounds(4);
            
            % calculate rect
            w = obj.prLayers('task_details').w;
            h = obj.prLayers('task_details').h;
            x = w - tw - 8;
            y = th + 2;
            
            % draw white with black shadow for readability
            if ~obj.DisableTextDrawing
                Screen('DrawText', obj.prLayers('task_details').ptr, str, x, y,...
                    [255, 255, 255, 255], [000, 000, 000, 50]);
            end
            
            % restore font size
            Screen('TextSize', obj.prLayers('task_details').ptr, prevTextSize);
            
        end
        
        function DrawCalibPoint(obj, pt)
            if nargin == 1
                pt = obj.ETCalibPoint;
            end
            if isempty(pt) || ~pt.Active
                return
            end
            
%             if obj.prDisableStimulusDrawing
                % clear texture
                obj.ClearLayer('et_calib_point')
                % convert normalised ET coords to pixels
                res = Screen('Rect', obj.prWindowPtr);
                px = pt.X * res(3);
                py = pt.Y * res(4);
                dia = pt.Diameter * res(3);
                rect = teRectFromDims(px, py, dia, dia);
                Screen('DrawTexture', obj.prLayers('et_calib_point').ptr,...
                    pt.Stim.TexturePtr, [], rect, pt.Rotation);
%             else
%                 % convert [x, y] coords and radius from noramlised coords to
%                 % cm
%                 centre = obj.ScalePoint([pt.X, pt.Y],...
%                     'rel2cm');
%                 dia = obj.ScaleValue(pt.Diameter, 'rel2cmx');
%                 rect = teRectFromDims(centre(1), centre(2), dia, dia);
%                 % draw
%                 obj.DrawStim(pt.Stim, rect, pt.Rotation);
%             end
%             pt.Update
        end        
        
        function DrawTiming(obj)
            col_fg = obj.COL_LABEL_FG;
            % timing - draw frame and label first
            if obj.DrawTimingOnPreview && obj.prTimingIdx ~= 1
                obj.ClearLayer('timing')
                ...
                Screen('FrameRect', obj.prLayers('timing').ptr, [0, 0, 0],...
                    rect_tg + 3, 3)
                Screen('FrameRect', obj.prLayers('timing').ptr, col_fg,...
                    rect_tg, 3)
                ...
                Screen('DrawLine', obj.prLayers('timing').ptr, [0, 255, 0], 0, targY,...
                    gw, targY, 3);
                str = sprintf('%.1fms', tar_ft);
                if ~obj.DisableTextDrawing
                    Screen('DrawText', obj.prLayers('timing').ptr, str,...
                        gw, targY, col_fg);
                end
                % draw
                Screen('DrawDots', obj.prLayers('timing').ptr, [x; y], 5, cols);   
%                 % draw texture to preview
%                 rect_timingTex = [res_monitor(1) - w_timing, res_monitor(2) - h_timing,...
%                     res_monitor(1), res_monitor(2)];
%                 Screen('DrawTexture', obj.prLayers('preview').ptr,...
%                     obj.prLayers('timing').ptr, [], rect_timingTex)
            end
        end
        ...
        % calculate rect for drawing
            ...
            innerRect = [0, 0, obj.LightPatchSize];
            outerRect = Screen('Rect', obj.MonitorNumber);
            rect = teRectFromPreset(innerRect, outerRect,...
                obj.LightPatchPositionPreset);
            
        % draw
        
            Screen('FillRect', obj.prLayers('light_patch').ptr, col, rect)
...
%             Screen('FillRect', obj.prWindowPtr, obj.prBackColour);
        ...
        function DrawStimSchematicsToPreview(obj, buf, buf_stim)
        % draw schematic representation of a stimulus to the preview. This
        % means drawing an orange square and labelling it with the stim
        % name/filename. 
        
            if isempty(buf) || isempty(buf_stim), return, end
            if size(buf, 1) ~= length(buf_stim)
                error('Size mismatch between draw buffer and buffered stimulus objects.')
            end
            
            numStim = size(buf, 1);
            
            fontSize = round(16 / obj.prPreviewScale);
            oldFontSize = Screen('TextSize', obj.prLayers('previewschema').ptr, fontSize);
          ...
          % reshape
            ...
            % draw frames
            Screen('FrameRect', obj.prLayers('previewschema').ptr, obj.COL_TE_DARKPURPLE,...
                rect, 2);
            Screen('FillRect', obj.prLayers('previewschema').ptr, [obj.COL_TE_PURPLE, 50],...
                rect, 2);
            
            % draw labels
            for s = 1:numStim
                
                % stimulus object label
                if ~isempty(buf_stim{s}.Name)
                    str_lab = buf_stim{s}.Name;
                elseif ~isempty(buf_stim{s}.Filename)
                    str_lab = buf_stim{s}.Filename;
                else
                    str_lab = 'Unamed Stimulus';
                end
                
                % stimulus status
                str_status = buf_stim{s}.State;
                
                % (if movie) current time
                if buf_stim{s}.isMovie &&...
                        ~isnan(buf_stim{s}.CurrentTime) &&...
                        ~isnan(buf_stim{s}.Duration)
                    str_mov = sprintf('%.1fs / %.1fs',...
                        buf_stim{s}.CurrentTime, buf_stim{s}.Duration);
                else
                    str_mov = '';
                end
                ...
                if ~obj.DisableTextDrawing
                    [~, ~, th] = Screen('DrawText', obj.prLayers('previewschema').ptr, str_lab,...
                        x, y, obj.COL_LABEL_FG);    
                    y = y + th;
                    [~, ~, th] = Screen('DrawText', obj.prLayers('previewschema').ptr, str_status,...
                        x, y, obj.COL_LABEL_FG);    
                    y = y + th;
                    [~, ~, ~] = Screen('DrawText', obj.prLayers('previewschema').ptr, str_mov,...
                        x, y, obj.COL_LABEL_FG);
                end
            end
            
            Screen('TextSize', obj.prLayers('previewschema').ptr, oldFontSize);            
        
        end
        
        function CompositeMainWindow(obj)
            obj.doComposite(obj.prWindowPtr, 'drawOnWindow', 'main');
        end
        
        function CompositePreviewWindow(obj)
            obj.doComposite(obj.prPreviewPtr, 'drawOnPreview', 'preview');
        end
        
        function UpdateStimuli(obj)
        % perform any updates for currently playing stimuli
        
            if isempty(obj.Stim), return, end
        ...
        function UpdateMovieStimulus(obj, stim)
            ...
            if obj.prDoSchematic
                [texPtrPrev, movTimePrev] = Screen('GetMovieImage', obj.prPreviewPtr,...
                stim.MoviePtr, 0);
            else
                texPtrPrev = nan;
                movTimePrev = nan;
            ...
            if obj.FullDebugOutput
                teEcho('Update stim %s, main ptr=%d, preview ptr=%d, movTime=%.2f, movTimePrev=%.2f\n',...
                    stim.Name, texPtr, texPtrPrev, movTime, movTimePrev);
            end
            
            % check that a frame was returned
            if texPtr == 0 && movTime ~= -1
            ...
            elseif texPtr == -1 || texPtrPrev == -1 ||...
                    stim.Duration - stim.CurrentTime < (1 / obj.FPS) 
            % EOF
            
                if obj.FullDebugOutput 
                    teEcho('End of video file...');
                end
                ...
                if obj.FullDebugOutput 
                    teEcho('Stopped video file\n');
                end                
                
            end
            
            % if a new preview texture has been returned, close the
            % previous frame's texture and assign this frame's texture to
            % the stim object. Even though in schematic mode we won't get
            % all frames of the movie, this will give a low frame rate
            % rendition in the preview window
            if obj.prDoSchematic && texPtrPrev > 0
                if ~isempty(stim.PreviewTexturePtr) &&...
                        ~isnan(stim.PreviewTexturePtr) &&...
                        stim.PreviewTexturePtr > 0
                    Screen('Close', stim.PreviewTexturePtr);
                end
                stim.PreviewTexturePtr = texPtrPrev;
            end
        ...
        function UpdatePreview(obj)  
            ...
            if obj.prDoComposite
                % copy draw pane to preview
                obj.ClearLayer('preview');
                Screen('DrawTexture', obj.prLayers('preview').ptr, obj.ptr); 
            else
                % todo
            end
...
%                 Screen('DrawTexture', obj.prLayers('preview').ptr,...
%                     obj.prLayers('et_calib').ptr);
%             end
%             
%             % draw gaze on preview
%             Screen('DrawTexture', obj.prLayers('preview').ptr, obj.prLayers('et_gaze').ptr)       
            
            % draw face to preview
            res_drawing = obj.Resolution;
            res_monitor = Screen('Rect', obj.MonitorNumber);
            res_monitor = res_monitor(3:4);
            rect_etface = Screen('Rect', obj.prLayers('et_face').ptr) .* .5;
            rect_etface = teRectFromPreset(rect_etface,...
                [0, 0, res_monitor], 'bottomleft');            
            Screen('DrawTexture', obj.prLayers('preview').ptr,...
                obj.prLayers('et_face').ptr, [], rect_etface);  
            
            % draw task details
            Screen('DrawTexture', obj.prLayers('preview').ptr,...
                obj.prLayers('task_details').ptr)
            
            % draw schematic
            Screen('DrawTexture', obj.prLayers('preview').ptr,...
                obj.prLayers('previewschema').ptr)
            
            % clear schematic
            obj.ClearLayer('previewschema');
        ...
        function ClearTexture(obj, texPtr, bgCol)
            % if not specified, bg colour is black/invisible, but if a
            % value is passed that will be used
            if ~exist('bgCol', 'var') || isempty(bgCol)
                bgCol = [0, 0, 0, 0];
            end
            
            Screen('BlendFunction', texPtr, GL_ONE, GL_ZERO);
            Screen('FillRect', texPtr, bgCol)
            Screen('BlendFunction', texPtr, obj.CONST_ALPHA_BLEND_FACTOR_SRC,...
                obj.CONST_ALPHA_BLEND_FACTOR_DEST);
        end
        
        function ClearDrawBuffer(obj)
            obj.prDrawBuffer = nan(obj.CONST_DEF_DRAW_BUFFER_SIZE, 8);
        ...
        % get screen dimensions
            ...
            w = obj.prLayers('screenflash').w;
            h = obj.prLayers('screenflash').h;
        ...
        % fade screen from black to white
            ...
            obj.ClearLayer('screenflash')
            obj.prLayers('screenflash').drawOnWindow = true;
            obj.prLayers('screenflash').drawOnPreview = true;
            ...
            while teGetSecs - onset < kf.Duration
                ...
                Screen('FillRect', obj.prLayers('screenflash').ptr, [lum, lum, lum]);
                obj.RefreshDisplay;
            end
        ...
        % draw
            ...
            % black background
            Screen('FillRect', obj.prLayers('screenflash').ptr, [000, 000, 000]);
            % corner markers
            Screen('FillRect', obj.prLayers('screenflash').ptr, col_mrk', rect_mrk');
            % qr
            Screen('DrawTexture', obj.prLayers('screenflash').ptr, tex_qr, [], rect_qr, [], 0);
            % text
            old_fontSize = Screen('TextSize', obj.prLayers('screenflash').ptr, 26);
            old_fontName = Screen('TextFont', obj.prLayers('screenflash').ptr, 'menlo');
            DrawFormattedText(obj.prLayers('screenflash').ptr, smryStr, rect_str(1), 'center',...
                [255, 255, 255], [], [], [], [], [], rect_str);
            Screen('TextSize', obj.prLayers('screenflash').ptr, old_fontSize);
            Screen('TextFont', obj.prLayers('screenflash').ptr, old_fontName);
            % refresh
            flipTime = obj.RefreshDisplay;
            % send marker
            obj.SendRegisteredEvent('SYNC', flipTime);
        ...
        % hold for 5s
            ...
            obj.prLayers('screenflash').drawOnWindow = false;
            obj.prLayers('screenflash').drawOnPreview = false;
...
%             end
            ...
            switch obj.WindowMode
                case {'fullscreen', 'monoscreenmode'}
                    % in either case the resolution of the drawing pane is
                    % that of the monitor 
                    res = res([3, 4]);
                    size_monitor = obj.MonitorSize;
                case 'borderless'
                    % here the resolution is that of the borderless window
                    % itself
                    [res, size_monitor] = obj.CalculateBorderlessDimensions;
                    obj.prBorderlessDimensions = size_monitor;
            end
            % calculate params
            obj.prCmPerPx = size_monitor ./ res;
            obj.prPxPerCm = res ./ size_monitor;
        ...
        function [res, size_monitor] = CalculateBorderlessDimensions(obj)
            
            if isempty(obj.BorderlessWindowPosition)
                % this shouldn't happen but just in case
                error('When DrawMode is ''borderless'' you must set the BorderelessWindowPosition property before scaling can be calculcated.')
            end
            
            res = obj.BorderlessWindowPosition([3, 4]) -...
                obj.BorderlessWindowPosition([1, 2]);       
            
            % calculate monitor size, by scaling the MonitorSize property
            % by the ratio of the width and height of the window to that of
            % the monitor
            rect_window = obj.BorderlessWindowPosition;
            w_window = rect_window(3) - rect_window(1);
            h_window = rect_window(4) - rect_window(2);
            % get fullscreen resolution
            res_monitor = Screen('Rect', obj.prMonitorNumber);
            % scale
            xscale = w_window / res_monitor(3);
            yscale = h_window / res_monitor(4);
            % apply to monitor size
            size_monitor = obj.MonitorSize .* [xscale, yscale];     
        ...
        function OpenWindow(obj)
            ...
            % handle init for different PreviewDrawModes
            if obj.prDoComposite
                % if compositing, we need to share OpenGL contexts across
                % windows
                Screen('Preference', 'ConserveVRAM', 256);
            elseif obj.prDoSchematic
                % todo
            end
...
%                 % open main window. Optionally support MonoScreenMode, which
%                 % opens a full resolution frame buffer but draws it at a
%                 % redcued size (.MonoScreenResolution)
...
%                             [], [], obj.CONST_MULTISAMPLE, [], [], fullScreenRect);
%                         % set preview monitor number to same screen as main
%                         % window
%                         obj.prPreviewMonitorNumber = obj.prMonitorNumber;      
%                     case 'borderless'
%                         % this code is longer, so has its own method
%                         obj.OpenBorderlessWindow
%                 end
                bgCol = obj.COL_TE_PURPLE;     
                ...
                Screen('BlendFunction', obj.prWindowPtr, obj.CONST_ALPHA_BLEND_FACTOR_SRC,...
                    obj.CONST_ALPHA_BLEND_FACTOR_DEST);
            ...
            % init standard layers
            obj.InitialiseLayers

            % init a texture to store drawing contents are each refresh
            obj.CurrentDrawing = Screen('OpenOffscreenWindow',...
                obj.prWindowPtr, [], [], [], [], obj.CONST_MULTISAMPLE);
            
%             % init a texture to use whilst compositing (due to limitations
%             % on drawing multisampled textures into windows) 
%             rect_comp = Screen('Rect', obj.ptr);
%             obj.prCompBuffer = Screen('OpenOffscreenWindow',...
%                 obj.prWindowPtr, [], rect_comp, [], [], []);
%             Screen('BlendFunction', obj.prCompBuffer, obj.CONST_ALPHA_BLEND_FACTOR_SRC,...
%                 obj.CONST_ALPHA_BLEND_FACTOR_DEST);
            ...
            Screen('DrawTexture', obj.prLayers('logo').ptr, texSplash);
            obj.RefreshDisplay            
            ...
            obj.prLayers('logo').drawOnWindow = false;
            obj.prLayers('logo').drawOnPreview = false;
...
%             Screen('FinalizeMovie', obj.prCapturePtr);
        ...
        function OpenDrawPane(obj)
            ...
            obj.UpdateScaling
            if obj.prWindowLimitEnabled
                % window limits are enabled, and the WindowLimitSize
                % property takes precedence
                res = [0, 0, round(obj.WindowLimitSize .* obj.prPxPerCm)];
            elseif strcmpi(obj.WindowMode, 'borderless')
                % borderless drawing mode, adjust the already-set 
                % MonitorSize property according to the
                % BorderlessWindowPosition property (essentially, scaling
                % it down)
                res = [0, 0, obj.CalculateBorderlessDimensions];
            ...
            % turn on alpha blending 
            Screen('BlendFunction', obj.ptr, obj.CONST_ALPHA_BLEND_FACTOR_SRC,...
                obj.CONST_ALPHA_BLEND_FACTOR_DEST);             
        ...
        function OpenPreview(obj)
            ...
            if obj.prPreviewInWindow 
                opt_windowOption = kPsychGUIWindow; 
            else
                opt_windowOption = [];
            end         
            % get preview position from preset
            if strcmpi(obj.WindowMode, 'borderless')
                % position presets don't work in borderless mode (at least
                % on linux where one window is spanned across monitors), so
                % use the explicit position set for this mode
                obj.prPreviewPosition = obj.BorderlessPreviewPosition;
            else
                % otherwise use the preset
                obj.prPreviewPosition = obj.WindowPositionFromPreset(...
                    obj.prPreviewPositionPreset, obj.prPreviewScale, 'monitor');
            end
            % open
            if strcmpi(obj.WindowMode, 'borderless')
                [mw, mh] = teWidthHeightFromRect([0, 0, obj.prBorderlessDimensions]);
                res_monitor = [0, 0, obj.ScalePoint([mw, mh], 'cm2px')];
            else
                res_monitor = Screen('Rect', obj.MonitorNumber);
            end
            opt_pixelSize = [];
            ...
            opt_multiSample = [];
            opt_imagingMode = [];
            opt_clientRect = res_monitor;
            obj.prPreviewPtr = Screen('OpenWindow',...
                obj.prPreviewMonitorNumber,...
                obj.BackColour,...
                obj.prPreviewPosition,...
                opt_pixelSize,...
                opt_numBuffers,...
                opt_stereoMode,...
                opt_multiSample,...
...
%             obj.prPreviewPtr = Screen('OpenWindow', 0, [], obj.prPreviewPosition);
...
%                 [], windowOption, [0, 0, obj.Resolution]);    
            % turn on alpha blending 
            Screen('BlendFunction', obj.prPreviewPtr, obj.CONST_ALPHA_BLEND_FACTOR_SRC,...
                obj.CONST_ALPHA_BLEND_FACTOR_DEST); 
            % draw BG colour
            Screen('FillRect', obj.ptr, obj.prBackColour);
        ...
        function OpenBorderlessWindow(obj)
            
            % ensure we have a valid position
            if isempty(obj.BorderlessWindowPosition) ||...
                    isempty(obj.BorderlessPreviewPosition)
                error('You must set the BorderlessWindowPosition and BorderlessPreviewPosition properties to a valid rect in pixels before opening a window with WindowMode set to ''borderless''.')
            end
            
            % open 
            obj.prWindowPtr = Screen('OpenWindow', obj.prMonitorNumber,...
                obj.COL_TE_PURPLE, obj.BorderlessWindowPosition, [], [], [],...
                obj.CONST_MULTISAMPLE);            
        ...
        function LoadStim(obj, file, name, task)
            ...
            for f = 1:numFiles
                ...
                try
                    ...
                    switch ERR_loadStim.message
                        case 'Keys must be unique.'
                            error('Cannot load a stimulus with the same name as an existing (already loaded) stimulus: %s',...
                                nameToAdd)
                        otherwise
                            rethrow(ERR_loadStim)
                    end
                end
%                 if isempty(task)
%                     teEcho('\t- %s\n', fileToAdd);
            ...
            for s = 1:numToPrepare
                ...
                switch stim.Type
                    case 'IMAGE'
                        ...
                        % make texture
                        texPtr_main = Screen('MakeTexture', obj.prWindowPtr,...
                            stim.ImageData);
                        if obj.prDoSchematic
                            % create texture for preview window
                            texPtr_preview = Screen('MakeTexture', obj.prPreviewPtr,...
                                stim.ImageData);
                        else
                            texPtr_preview = nan;
                        end
                        % update flags
                        stim.SetPrepared(texPtr_main, texPtr_preview);
...
%                             stim.Filename, 4);
                        % get first frame
                        Screen('SetMovieTimeIndex', movPtr, 0);
                        texPtr_main = Screen('GetMovieImage', obj.prWindowPtr,...
                            movPtr);
                        ...
                        if obj.prDoSchematic
                            % create texture for preview window
                            texPtr_preview = Screen('GetMovieImage', obj.prPreviewPtr,...
                            movPtr);
                        else
                            texPtr_preview = nan;
                        end       
                        stim.MoviePtr = movPtr;
                        stim.SetPrepared(texPtr_main, texPtr_preview);
                    case 'SOUND'     
                ...
                if obj.FullDebugOutput
                    teEcho('Debug: spent %.0fms making texture for %s\n', toc * 1000, stim.Filename);                
                end                
            end
        end
...
%                         % if this is a movie, after setting the time back
%                         % to zero, get the frame at time zero so there is a
%                         % texture ptr available if someone tries to draw it
%                         % before pulling a new frame
%                         if stim.isMovie
%                             stim.TexturePtr = Screen('GetMovieImage',...
%                                 obj.prWindowPtr, stim.MoviePtr);
%                         end
%                         if stim.TexturePtr == -1
%                             error('Could not get first frame from movie')
%                         end                        
%                     end
                    ...
                    if obj.FullDebugOutput
                        teEcho('Attempting to stop movie...');
                    end
                    ...
                    stim.LastTouched = teGetSecs;                        
                otherwise
                    warning('Not yet implemented - %s!', stim.Name)
        ...
        function StopAllStim(obj)
            for i = 1:obj.Stim.Count
                obj.StopStim(obj.Stim(i));
            end
        end
        ...
        % plays a visual attention getter - for now this is a fixation
            ...
            if obj.EyeTracker.Valid
                teFixation(obj, 'useeyetracker', true);
            else 
                teFixation(obj);
            end
            obj.KeyFlush;
        ...
        % enable auto off in future
            ...
            if obj.LightPatchStatus
                ...
                obj.DrawLightPatch
                obj.prLayers('light_patch').drawOnWindow = true;
                obj.prLayers('light_patch').drawOnPreview = true;
                % log
                obj.AddLog(...
                    'source',       'presenter',...
                    'topic',        'lightpatch',...
        ...
        function ETGetEyes(obj)
            if ~obj.EyeTracker.Valid
                warning('Cannot get eyes when eye tracker is missing or not in a valid state.')
                return
            end
            if strcmpi(obj.EyeTracker.TrackerType, 'mouse')
            ...
            obj.AddLog(...
                ...
                'data',         'geteyes_onset');            
            % don't draw calib points, which may still be active from a
            % previous calibration that was cancelled to return to get eyes
            obj.prLayers('et_calib_point').drawOnWindow = false;
            obj.prLayers('et_calib_point').drawOnPreview = false;            
            % draw et face
            obj.DrawETFaceOnMainWindow = true;
            % prepare movie
        ...
        function calibHappy = ETCalibrate(obj)
            ...
            obj.AddLog(...
                'source',       'presenter',...
                'topic',        'et_full_calibration',...
                'data',         'calib_onset');  
            
            % message user
            teTitle('Eye tracker calibration:\n\n');
            msg{1} = 'The eye tracker will first measure all calibration points and compute';
            msg{2} = 'an initial calibration. Any points that fail to meet accuracy and precision';
            msg{3} = 'criteria will be retried automatically. This will continue until either all';
            msg{4} = 'points are valid, or until you choose to accept the current calibration and';
            msg{5} = 'move on.';
            teEcho('%s\n', msg{:});
            
            % set BG colour and window limit border to same colour, store
            % current colours to restore afterwards
            old_bgCol = obj.BackColour; 
            old_wlCol = obj.WindowLimitBorderColour;
            obj.BackColour = obj.ETCalibBackgroundColour;
            obj.WindowLimitBorderColour = obj.ETCalibBackgroundColour;
            
            % make calibration layer visible
            obj.prLayers('et_calib_point').drawOnWindow = true;
            obj.prLayers('et_calib_point').drawOnPreview = true;
            obj.prLayers('et_calib').drawOnPreview = true;
            
            % clear calib results drawing 
            obj.ClearLayer('et_calib');
            
            % define calib points and schedule, and randomise the order
            obj.prETCalibDef = [...
                                    0.1,    0.1;    ...
                                    0.9,    0.1;    ...
                                    0.5,    0.5;    ...
            ...
            obj.prETCalibDef =...
                obj.prETCalibDef(randperm(size(obj.prETCalibDef, 1)), :);
            
            % create a calibration point object and run the schedule
            calibHappy = false;
            while ~calibHappy
                
                pt = teCalibrationPoint_eyeTracker(...
                    obj, obj.prETCalibDef, obj.EyeTracker);
                [suc, oc] = pt.RunSchedule;
                
                if ~suc
                    
                    % detect if ESC has been pressed to go back to get eyes
                    % screen
                    if strcmpi(oc, 'get_eyes')
                        teEcho('Exiting calibration, going back to get eyes screen.\n');
                        calibAccepted = false;
                        return
                    end
                        
                    
                    % calibration failed -- should we try again?
                    teEcho('%s\n', oc);
                    resp = input('Do you want to continue with an imperfect calibration (y) or retry (n)?', 's');
                    
                else
                    
                    % calibration succeeded -- accept it or try again?
                    teEcho('%s\n', oc); 
                    resp = input('Do you want to accept this calibration (y) or retry (n)?', 's');
                    
                end
                
                calibHappy = contains(lower(resp), 'y');
            ...
            % hide calibration layer 
            obj.prLayers('et_calib_point').drawOnWindow = false;
            obj.prLayers('et_calib_point').drawOnPreview = false;
            obj.prLayers('et_calib').drawOnPreview = false;
            
            % restore BG/window limit colours
            obj.BackColour = old_bgCol;
            obj.WindowLimitBorderColour = old_wlCol;
            
        end
        
        function skipped = ETCalibrate_Initial(obj, pt)
        % performs an initial n-point calibration, which allows the eye
        % tracker to compute an initial calibration. The only way in which
        % this will fail is if the eye tracker itself decides that the data
        % is too poor. For that reason, this method will keep trying to
        % gather a valid (according to the eye tracker) calibration, unless
        % the experimenter chooses to give up. Note that
        % ETCalibrate_Improve should be calle after this method -- this
        % will judge the calibration on accuray/prec at each point and
        % attempt to improve noisy points. 
            ...
            obj.AddLog(...
                'source',       'presenter',...
                'topic',        'et_initial_calibration',...
                'data',         'calib_onset');              
            
            msg{1} = '\tStage 1 (initial calibration):\n';
            msg{2} = '\tCollecting data for initial calibration. A calibration has not yet';
            msg{3} = '\tbeen computed.';
            teEcho('\n%s', msg{:});
            teEcho(' Press %s to accept this invalid calibration and move on. Press \n\t%s to go back to detecting eyes\n\n',...
                obj.KB_MOVEON, obj.KB_MOVEBACK);
            
            % set screen up for calibration
            obj.DrawCalibOnPreview = true;
            obj.BackColour = obj.ETCalibBackgroundColour;
            obj.RefreshDisplay;        
            obj.KeyFlush;
            
            % loop until valid calibration
            while ~pt.ETLastCalibrationSuccess &&...
                    ~obj.KeyPressed(obj.KB_MOVEON) &&...
                    ~obj.KeyPressed(obj.KB_MOVEBACK)
                
                % clear current calib and run calib schedule
                obj.EyeTracker.ClearCalibPoints;
                obj.DrawCalibrationResults;
                pt.RunSchedule;
                while pt.ScheduleRunning && ~obj.KeyPressed(obj.KB_MOVEON) &&...
                        ~obj.KeyPressed(obj.KB_MOVEBACK)
                    pt.Update
                    obj.DrawCalibPoint(pt)
                    obj.RefreshDisplay;
                end
                
                % determine calib success
                if pt.ETLastCalibrationSuccess
                    teEcho('\tSuccessfully computed a initial calibration.\n');
                else
                    teEcho('\tAttempt did not yield enough data for a valid calibration, retrying\n');
                end
        ...
        function skipped = ETCalibrate_Improve(obj, pt)
        % this must be run after ETCalibrate_Initial -- this collects data
        % and computes an initial calibration on the eye tracker. The
        % purpose of ETCalbrate_Improve is to recalibrate noisy points that
        % fail our data quality criteria (set by the 
        % ETCalibMinValidAccuracyDeg and ETCalibMinValidPrecisionDeg
        % properties). This method will keep trying until all points meet
        % both criteria, or the experimenter choooses to move on
            ...
            % define calib schedule table
            cs = array2table([pt.Schedule(:, 1), pt.Schedule(:, 2),...
                false(size(pt.Schedule, 1), 1),...
                true(size(pt.Schedule, 1), 1)],...
                'VariableNames', {...
                'PointX',...
                'PointY',...
                'Valid',...
            ...
            % sumamrise current calibration
            cs = obj.ETSummariseCalibration(...
                cs, pt.ETLastCalibrationOnset, pt.ETLastCalibrationOffset);
            obj.DrawCalibrationResults;
            obj.RefreshDisplay;   
                
            teEcho('\n\n\tStage 2 (improve noisy points):\n\n');
            
            % if all points are valid, we're done here
            if all(cs.Valid)
                teEcho('\tAll points on the initial calibration met criteria. Calibration complete.\n');
                teLine;
                return
            end
            ...
            skipped = false;
            while ~all(cs.Valid) && ~skipped
                
                idx_invalid = ~cs.Valid;
                numInvalid = sum(~cs.Valid);
                teEcho('\t%d of %d points did not meet criteria. Recalibrating those points.\n',...
                    numInvalid, size(cs, 1));                
                
                % filter schedule for just the noisy points
%                 pt.Schedule = cs{idx_invalid, {'PointX', 'PointY'}};
                pt.Schedule = cs{:, {'PointX', 'PointY', 'Valid'}};
                ...
                % detect keypress to skip calib
                skipped = obj.KeyPressed(obj.KB_MOVEON) ||...
                    obj.KeyPressed(obj.KB_MOVEBACK);
                
                cs = obj.ETSummariseCalibration(cs,...
                    pt.ETLastCalibrationOnset, pt.ETLastCalibrationOffset);
                
%                 % determine calib success
%                 cs_tmp = array2table([pt.Schedule(:, 1), pt.Schedule(:, 2),...
%                     false(size(pt.Schedule, 1), 1),...
%                     true(size(pt.Schedule, 1), 1)],...
%                     'VariableNames', {...
%                     'PointX',...
...
%                     'Measured',...
%                     });
%                 cs(idx_invalid, :) = cs_tmp;
% 
%                 % sumamrise current calibration
%                 cs(idx_invalid, :) = obj.ETSummariseCalibration(...
%                     cs(idx_invalid, :), pt.ETLastCalibrationOnset, pt.ETLastCalibrationOffset);
                obj.DrawCalibrationResults;
                obj.RefreshDisplay;
        ...
        % precision statistics
            ...
            gaze = arrayfun(@(from, to) obj.EyeTracker.GetGaze(from, to),...
                from, to, 'UniformOutput', false);
            dist = cellfun(@(x) x(:, [12, 27]), gaze, 'UniformOutput', false);
            medDist = nanmean(cellfun(@(x) nanmedian(x(:)) / 10, dist));
...
%             end
        ...
        function DrawCalibrationResults(obj, pt)
            if ~obj.WindowOpen
                error('Window not open.')
            end
            % set up colours
            ...
            % clear texture
            obj.ClearLayer('et_calib');
            Screen('TextSize', obj.prLayers('et_calib').ptr,...
                obj.prPreviewScaledFontSize);
            % draw semi-transparent background to darken existing stim
            Screen('FillRect', obj.prLayers('et_calib').ptr, [0, 0, 0, 200]); 
            ...
            if ~exist('pt', 'var') || isempty(pt)
                ...
                % convert point data to table
                tab = table;
                numPoints = size(pt.Schedule, 1);
                for p = 1:numPoints

                    % if not yet measured, skip
                    if ~pt.Schedule.measured(p), continue, end

                    % if not data (prob beacause calibration of this point
                    % failed)
                    if isnan(pt.Schedule.accuracy_l(p)) &&...
                            isnan(pt.Schedule.accuracy_r(p))
                        continue
                    end

                    gaze = pt.Schedule.gaze{p};
                    numGaze = size(gaze, 1);
                    tmp = table;

                    tmp.PointX = repmat(pt.Schedule.x(p), numGaze, 1);
                    tmp.PointY = repmat(pt.Schedule.y(p), numGaze, 1);
                    tmp.LeftX = gaze(:, 1);
                    tmp.LeftY = gaze(:, 2);
                    tmp.RightX = gaze(:, 3);
                    tmp.RightY = gaze(:, 4);
                    tmp.LeftValidity = repmat(pt.Schedule.valid(p), numGaze, 1);
                    tmp.RightValidity = repmat(pt.Schedule.valid(p), numGaze, 1);
                    tmp.LeftOffsetX = pt.Schedule.offset_l_x{p};
                    tmp.LeftOffsetY = pt.Schedule.offset_l_y{p};
                    tmp.RightOffsetX = pt.Schedule.offset_r_x{p};
                    tmp.RightOffsetY = pt.Schedule.offset_r_y{p};
                    tmp.LeftOffset = pt.Schedule.offset_l{p};
                    tmp.RightOffset = pt.Schedule.offset_l{p};

                    tab = [tab; tmp];

                end
    
                calib_val = ~isempty(tab);
                
            end
            
            if ~calib_val
                if ~obj.DisableTextDrawing
                    DrawFormattedText(obj.prLayers('et_calib').ptr,...
            ...
            % calib points
            w = obj.prLayers('et_calib').w;
            h = obj.prLayers('et_calib').h;
            pt_x_all = tab.PointX .* w;
            pt_y_all = tab.PointY .* h;
            pt_x = pt.Schedule.x * w;
            pt_y = pt.Schedule.y * h;

            % convert x, y to rect for drawing by PTB
            pt_rect = zeros(4, numPoints);
            for p = 1:numPoints
                pt_rect(:, p) =...
                    teRectFromDims(pt_x(p), pt_y(p), sz_pt, sz_pt)';
            end                
            % draw points
            Screen('FillOval', obj.prLayers('et_calib').ptr, col_pt, pt_rect, 3);

            % gaze points
            g_l = [tab.LeftX * w, tab.LeftY * h]';
            g_r = [tab.RightX * w, tab.RightY * h]';
            % colours - left 
            val_l = tab.LeftValidity;
            ...
            % draw gaze 
            Screen('DrawDots', obj.prLayers('et_calib').ptr, g, sz_g, g_col,...
                [], 2);
            ...
            % draw quiver
            g_col(4, :) = 30;
            Screen('DrawLines', obj.prLayers('et_calib').ptr, quiv_coords,...
                2, [g_col, g_col]);

            % draw accuracy and precision
            alx = pt.Schedule.centroid_l_x;
            aly = pt.Schedule.centroid_l_y;
            arx = pt.Schedule.centroid_r_x;
            ary = pt.Schedule.centroid_r_y;
            prec_lxn = pt.Schedule.precision_l;
            prec_lyn = pt.Schedule.precision_l;
            prec_rxn = pt.Schedule.precision_r;
            prec_ryn = pt.Schedule.precision_r;            

            % rescale to pixels
            apos_l  = [alx * w, aly * h];
            apos_r  = [arx * w, ary * h];
            aw_l    = [prec_lxn * w, prec_lyn * h];
            aw_r    = [prec_rxn * w, prec_ryn * h];
            ...
            Screen('FrameOval', obj.prLayers('et_calib').ptr, col_val_l,...
                vertcat(arect_l{:})', 3);
            Screen('FrameOval', obj.prLayers('et_calib').ptr, col_val_r,...
                vertcat(arect_r{:})', 3);     

            % print accuracy and precision
            val     = pt.Schedule.valid;
            % get vars
            acc_l   = pt.Schedule.accuracy_deg_l;
            prec_l   = pt.Schedule.precision_deg_l;
            acc_r   = pt.Schedule.accuracy_deg_r;
            prec_r   = pt.Schedule.precision_deg_r;            
            ...
            halign  = repmat({'left'}, length(x), 1);
            halign(x > .51) = repmat({'right'}, sum(x > .51), 1);
            halign(x == .5) = repmat({'center'}, sum(x == .5), 1);
            % colour by validity
            tcol    = repmat({col_pt}, numel(val), 1);
            tcol(~val) = repmat({[220, 000, 000]}, sum(~val), 1);
            % make string
            tdeg = ' deg';
            tstr = cell(numPoints, 1);
            for p = 1:numPoints
                
                if ~pt.Schedule.measured(p) || (isnan(acc_l(p)) && isnan(acc_r(p)))
                    tstr{p} = 'Measuring...';
                else
                    tstr{p} = sprintf('Left: %.2f%s (%.2f%s)\nRight: %.2f%s (%.2f%s)',...
                        acc_l(p), tdeg, prec_l(p), tdeg, acc_r(p),...
                        tdeg, prec_r(p), tdeg);
                end
...
%                 acc_l, prec_l, acc_r, prec_r, 'uniform', false);
            % draw shadow
            cellfun(@(str, rect, halign) DrawFormattedText(obj.prLayers('et_calib').ptr,...
                str, halign, 'center', [000, 000, 000], [], [], [], [], [],...
                rect + 1), tstr, trect, halign);                
            % draw text
            cellfun(@(str, rect, col, halign) DrawFormattedText(obj.prLayers('et_calib').ptr,...
                str, halign, 'center', col, [], [], [], [], [],...
                rect), tstr, trect, tcol, halign);           

        end
        
        function DrawCalibrationResults_old(obj, pt)
            if ~obj.WindowOpen
                error('Window not open.')
            end
            % set up colours
            ...
            if obj.EyeTracker.Calibrated 
                ...
                pt_x_all = tab.PointX .* w;
                pt_y_all = tab.PointY .* h;
                pt_x = obj.EyeTracker.Calibration.Points(:, 1) .* w;
                pt_y = obj.EyeTracker.Calibration.Points(:, 2) .* h;
                % convert x, y to rect for drawing by PTB
                ...
                % compute text position
                x       = obj.EyeTracker.Calibration.Summary.PointX;
                y       = obj.EyeTracker.Calibration.Summary.PointY;
                tx      = x * w;
                ty      = (y + .05) * h;
                tw      = .2 * w;
                th      = .2 * h;
        ...
        function DrawCalibrationPoints(obj)
            if isempty(obj.ETCalibPoint)
                return
            end
        end
        
        function ResetCalibPoint(obj)
            obj.ETCalibPoint = teCalibrationPoint_posthoc(obj);
        end
        
        function calibAccepted = ETCalibrateOld(obj)
            if ~obj.EyeTracker.Valid
                error('Eye tracker not in valid state.')
            end
            obj.AddLog(...
...
%             obj.Tracker.EyeTrackerCalibration = obj.EyeTracker.Calibration;
        ...
        function ETPosthocCalibration(obj)
            
            teEcho('Performing post-hoc calibration...\n');
            ...
            obj.ETCalibPoint.RunSchedule;
            while obj.ETCalibPoint.ScheduleRunning 
                obj.DrawCalibPoint
                obj.RefreshDisplay;
            end
            obj.prETPHCLastOffset = teGetSecs;
            obj.ETPosthocCalibrationResults = obj.ETCalibPoint;
        ...
        function AssignKeyboard(obj, assignedName, keyboardName)
            ...
            if obj.prKeyboardAssignment.Count == 1
...
%             end
        ...
        function [comp, map, taskKey, trialCounter] =...
            ...
            if list.RecalculateSamplingEachTime
                if obj.FullDebugOutput
                    teEcho('(Re)calculating sampling for list %s\n',...
                        list.Name);
                end
            ...
            elseif s2 > list.NumSamples
                warning('End sample of %d is beyond the size of the list (%d), update list NumSamples to %d',...
                    s2, list.NumSamples, s2)                
                list.NumSamples = numSamples;
            elseif s1 < 1
...
%                     end
                ...
                otherwise
                    ...
                    error('Expected trial type to be ''trial'', ''function'', ''eck'', or ''nestedlist''.')
...
%                     tc = tc + 1;
        ...
        function ExecuteList(obj, val, startSample, numSamples)
            ...
            if ~obj.prSessionStarted
                warning('Cannot execute a list until a session has started.')
            ...
            % case the session will stop on an error. 
                ...
                switch lower(comp{i}.Type)
                    ...
                    otherwise
                        ...
                        if obj.FullDebugOutput
                            teEcho('About to execute trial function %s\n', vars.Target);
                        end
                        if obj.HandleTrialErrors
                            try
                        ...
                        if obj.FullDebugOutput
                            teEcho('Finished executing trial function\n');
                        end                        
...
%                             vars(it).Presenter_TrialNo = thisTask.TrialNo;
            ...
            % offset event
                ...
                % process post-hoc calibration
                phc_durSinceLast = teGetSecs - obj.prETPHCLastOffset;
                if obj.ETPosthocCalibrationEnabled &&...
                        thisTask.NextTaskWillChange &&...
                        phc_durSinceLast > obj.ETPosthocCalibrationFrequencySecs
                    obj.ETPosthocCalibration
                end
                disp([phc_durSinceLast, obj.ETPosthocCalibrationFrequencySecs])
                    
                % perform trial end action
                switch lower(thisTask.TrialEndAction)
                    case 'continue'
                        % continue through the list - this is the default
                        % action 
                        i = i + 1;
                    case 'repeat'
...
%                         i = i + 1;
        ...
        % list they are. 
            ...
            % display
            if obj.ClearCommandWindowDuringListExecution
                clc
            end
        ...
        % appropriate relay exists and send it to that
            ...
            obj.LogEvent(event, when);
            
        end
        
        function when = SendRegisteredEvent(obj, key, when)
        ...
        % relay. 
            ...
            % extract task from event, if present
            if isfield(event, 'task')
                task = event.task;
            else
                task = [];
            end
...
%                 match = find(ismember(relay_name, types));
                ...
                % send event
                relay.SendEvent(eventToSend, when, task);
                obj.LogEvent(eventToSend, when);
            end
            
            
        end
        
        function LogEvent(obj, event, when)
        % updates the CurrentVariables struct (which contains the vars 
        % currently in use for a particular trial) with an event timestamp.
        % For example, if we .SendEvent('MY_EVENT', 1000), this method
        % creates a field such that vars.MY_EVENT=1000. 
        %
        % this doesn't work well with an array of variable structs (which
        % happens in tasks that create a trial loop and keep incrementing
        % the CurrentVariables struct as an array) so we dont do this in
        % that case
        
            if isempty(event) || isempty(obj.CurrentVariables) ||...
                    ~isscalar(obj.CurrentVariables), return, end
...
%             obj.FlushBuffer 
        ...
        function FlushBuffer(obj)
            ...
            if obj.EyeTracker.Valid && ~isempty(obj.Tracker.Path_EyeTracker)
...
%         end
        ...
        function AddLog(obj, varargin)
            ...
            if ~all(cellfun(@isstruct, varargin)) &&...
                    ~all(cellfun(@(x) isa(x, 'hstruct'), varargin))
                % build struct
                try
            ...
            for i = 1:numItems
                ...
                if ~isfield(s{i}, 'source')
                    dbs = dbstack;
                    % if called from the command window, dbs will be one
                    % element long, otherwise the source will be in the
                    % second element
                    if length(dbs) == 1
                        s{i}.source = 'command_window';
                    else
...
%                 ':CodecType=ffenc_ljpeg');
        ...
        % debug
        function set.CurrentVariables(obj, val)
            obj.CurrentVariables = val;
            if obj.FullDebugOutput
                teTitle('Update current vars\n\n');
                disp(val)
                teLine
            end
        end
        ...
        function set.MonitorNumber(obj, val)
            ...
            % if requesting monitor 0 (presumably the one with Matlab on
            % it) in fullscreen WindowMode, check with the user if they are
            % sure, since the screen will occlude the Matlab GUI
            if obj.prMonitorNumber == 0 && strcmpi(obj.prWindowMode, 'fullscreen')
                teLine
                teEcho('You have set MonitorNumber to 0, but WindowMode to ''fullscreen''. This will open a window\nfullscreen on your main monitor. Do you want to set MonoScreenMode\nin order to present stimuli in a small window (y/n)? >');
                resp = input('', 's');
        ...
        function set.DisableOpenGLErrorChecking(obj, val)
            if val
                Screen('Preference','ConserveVRAM', 512)
                warning('This may cause unintended behaviour.')
            end
            obj.DisableOpenGLErrorChecking = val;
        end
        ...
        function val = get.DrawingSize(obj)
            % if window limits are enabled, these take precedence (since in
            % theory window limits could cause us to draw to a LARGER pane
            % than the monitor itself). Otherwise, the answer depends on
            % the current DrawingMode. If fullscreen/monoscreenmode then
            % this is the set-at-window-open monitor size. If borderless,
            % we use the calculated-in-UpdateScaling borderless dimensions
            if obj.WindowLimitEnabled
                ...
                switch obj.WindowMode
                    case {'fullscreen', 'monoscreenmode'}
                        val = obj.prMonitorSize;
                    case 'borderless'
                        val = obj.prBorderlessDimensions;
                end
            end
        end
...
%                 end
                ...
                obj.prWindowMode = 'monoscreenmode';
                if obj.prWindowOpen
                    obj.ReopenWindow;
                end
            end
        ...
        function set.WindowLimitBorderColour(obj, val)
            ...
            obj.prWindowLimitBorderColour = val;
            % log
            obj.logWindowLimits            
        end
...
%             end
            ...
            if changed && obj.prWindowOpen, obj.ReopenPreview; end
        end
        ...
        function set.PreviewScale(obj, val)
            ...
            obj.prPreviewScaledFontSize = round(16 / obj.prPreviewScale);
        end
...
%             end
        ...
        function val = get.PreviewWindowPtr(obj)
            val = obj.prPreviewPtr;
        end
        
        function set.PreviewDrawMode(obj, val)
            switch val
                case 'schematic'
                    obj.prDoSchematic = true;
                    obj.prDoComposite = false;
                case 'composite'
                    obj.prDoSchematic = false;
                    obj.prDoComposite = true;
                otherwise
                    error('PreviewDrawMode must be either ''composite'' or ''schematic''.')
            end     
            obj.prPreviewDrawMode = val;                    
        end
        
        function val = get.PreviewDrawMode(obj)
            val = obj.prPreviewDrawMode;
        end
        
        function set.WindowMode(obj, val) 
            
            AssertValueIsInList(val,...
                {'fullscreen', 'borderless', 'monoscreenmode'});
            obj.prWindowMode = val;
            
            % handle the possible selection of monoscreen mode
            if strcmpi(val, 'monoscreenmode')
                obj.prMonoScreenMode = true;
            end
        ...
        function val = get.WindowMode(obj)
            val = obj.prWindowMode;
        end
        
        function val = get.BorderlessWindowPosition(obj)
            val = obj.prBorderlessWindowPosition;
        end
        
        function set.BorderlessWindowPosition(obj, val)
            obj.AssertRectType(val);
            obj.prBorderlessWindowPosition = val;
            obj.UpdateScaling;
        end
        
        function val = get.BorderlessPreviewPosition(obj)
            val = obj.prBorderlessPreviewPosition;
        end
        
        function set.BorderlessPreviewPosition(obj, val)
            obj.AssertRectType(val);
            obj.prBorderlessPreviewPosition = val;
%             obj.UpdateScaling;
        end        
                
        % layers
        function set.DrawGazeOnMainWindow(obj, val)
            obj.DrawGazeOnMainWindow = val;
            obj.UpdateLayerVisibility('et_gaze', 'mainWindow', val);
        end
        
        function set.DrawGazeOnPreview(obj, val)
            obj.DrawGazeOnPreview = val;
            obj.UpdateLayerVisibility('et_gaze', 'preview', val);
        end
        
        function set.DrawETFaceOnMainWindow(obj, val)
            obj.DrawETFaceOnMainWindow = val;
            obj.UpdateLayerVisibility('et_face', 'mainWindow', val);
        end
        
        function set.DrawCalibOnMainWindow(obj, val)
            obj.DrawCalibOnMainWindow = val;
            obj.UpdateLayerVisibility('et_calib', 'mainWindow', val);
        end        
        
        function set.DrawCalibOnPreview(obj, val)
            obj.DrawCalibOnPreview = val;
            obj.UpdateLayerVisibility('et_face', 'preview', val);
        end   
        
        function set.DrawTimingOnPreview(obj, val)
            obj.DrawTimingOnPreview = val;
            obj.UpdateLayerVisibility('timing', 'preview', val);
        end           
              
        function set.DrawPreviewOnMainWindow(obj, val)
            obj.DrawPreviewOnMainWindow = val;
            obj.UpdateLayerVisibility('preview', 'mainWindow', val);
        end   
...
%             end
        ...
        function set.EyeTracker(obj, val)
            ...
            if strcmpi(obj.EyeTracker.TrackerType, 'mouse')
                obj.EyeTracker.MouseWindowPtr = obj.prWindowPtr;
                obj.EyeTracker.SampleRate = obj.FPS;
            end
            % update eye tracker with new window limits
            obj.EyeTracker.UpdateWindowLimit(obj.prWindowLimitEnabled,...
                obj.MonitorSize, obj.prWindowLimitSize)             
...
%             obj.EyeTracker.PosixTimeOffset = obj.PosixTimeOffset;
        ...
        function set.LightPatchEnabled(obj, val)
            ...
            obj.UpdateLayerVisibility('light_patch', 'mainWindow', true)
            obj.UpdateLayerVisibility('light_patch', 'preview', true)
        end
        
        function set.LightPatchPositionPreset(obj, val)
            if ~ismember(lower(val), {...
                    'topleft',...
                    'bottomleft',...
                    'topright',...
                    'bottomright'})
                error('Valid LightPatchPositionPreset values are: topleft, bottomleft, topright, bottomright.')
            end
            obj.LightPatchPositionPreset = val;
        end
...
%             Screen('FillRect', obj.ptr, col_bg);
            ...
            Screen('FrameOval', obj.ptr, col_fg, crect, 2);
            ...
            Screen('DrawLines', obj.ptr, [xtp, ytp], 4, col_fg);
            
            % draw labels at 1cm intervals along smallest dim
            xlp = [cx - ticks', repmat(cy, length(ticks), 1)];
            ...
            ty = 0;
            [~, ~, th] = Screen('DrawText', obj.ptr, 'All sizes in cm.', 0, ty, col_fg);
            ty = ty + th;
            str = sprintf('Monitor number: %d', obj.MonitorNumber);
            [~, ~, th] = Screen('DrawText', obj.ptr, str, 0, ty, col_fg);
            
            ty = ty + th;
            str = sprintf('Monitor dimensions: %.2fcm x %.2fcm',...
                obj.MonitorSize(1), obj.MonitorSize(2));
            [~, ~, th] = Screen('DrawText', obj.ptr, str, 0, ty, col_fg);
            
            ty = ty + th;
            str = sprintf('Drawing pane dimensions: %.2fcm x %.2fcm',...
                obj.DrawingSize(1), obj.DrawingSize(2));
            [~, ~, ~] = Screen('DrawText', obj.ptr, str, 0, ty, col_fg);
            
            ty = ty + th;
            str = sprintf('Monitor resolution: %dpx x %dpx',...
...
%             end
        ...
        function DrawLayerThumbnails(obj)
            if ~obj.WindowOpen
                error('Window not open')
            end
            % check that there are some image/movie stimuli to draw
            if isempty(obj.prLayers)
                return
            end
            layers = copyHandleClass(obj.prLayers);
            keys = layers.Names';
            
            % split into main/preview
            if obj.prDoSchematic
                layers = obj.prLayers.Items;
                idx_preview = cellfun(@(x) strcmpi(x, 'preview'),...
                    layers(:, 8));
                l_main = layers(~idx_preview, :);
                names = obj.prLayers.Names;
                k_main = names(~idx_preview);
                l_preview = layers(idx_preview, :);
                k_preview = names(idx_preview);
            else
                l_main = obj.prLayers.Items;
                k_main = obj.prLayers.Names;
                l_preview = [];
                k_preview = [];
            end
            
            doDrawLayerThumbnails(l_main, k_main, obj.prWindowPtr)
            doDrawLayerThumbnails(l_preview, k_preview, obj.prPreviewPtr)

            function doDrawLayerThumbnails(layers, keys, ptr)
                
                % calculate positions
                numLayers = size(layers, 1);
                tb = numSubplots(numLayers);
                res = Screen('Rect', ptr);
                res = res(3:4);
                w = res(1) / tb(1);
                h = res(2) / tb(2);
                ...
                border = 20;
                % draw 
                Screen('FillRect', ptr, [000, 000, 000]);
                oldTextSize = Screen('TextSize', ptr, 36);
                row = 1;
                col = 1;
                for s = 1:numLayers
                    % make rect in cm
                    rect_px = [x(col) + border, y(row) + border,...
                        x(col) + w - border, y(row) + h - border];
                    % draw thumbnail
                    Screen('DrawTexture', ptr, layers{s, 1}, [], rect_px)
                    % draw border
                    Screen('FrameRect', ptr, obj.COL_TE_DARKPURPLE,...
                        rect_px, 3);
                    % draw text
                    txt = keys{s};
                    tx = rect_px(1);
                    ty = rect_px(2);
                    Screen('DrawText', ptr, txt, tx, ty,...
                        obj.COL_LABEL_FG, obj.COL_LABEL_BG);                    
                    % process row, col
                    col = col + 1;
                    if col > tb(1)
                ...
                Screen('Flip', ptr);
                Screen('TextSize', ptr, oldTextSize);
        ...
        function val = WindowPositionFromPreset(obj,...
            ...
            if ~exist('scaleReference', 'var') || isempty(scaleReference)
                scaleReference = 'drawing';
            end
            % get screen size and calculate screen ar
            res_drawingWindow = [0, 0, obj.Resolution];
            res_monitor = Screen('Rect', obj.MonitorNumber);
%             res_drawingWindow = obj.ScalePoint(obj.DrawingSize, 'cm2px');

            switch obj.WindowMode
                case 'borderless'
                    res_previewWindow = res_drawingWindow;
                otherwise
            ...
            switch scaleReference         
                case 'monitor'
                    w = round(res_monitor(3) * scale);
                    h = round(res_monitor(4) * scale);       
                otherwise
...
%             h = round(res_drawingWindow(2) * scale);
        ...
        function AssertRectType(~, val)
            ...
            if val(3) <= val(1) || val(4) <= val(2)
                % todo this is a warning for now in case it breaks anything
                warning('Impossible rect values, width and/or height is negative.')
            end
        end
...
%             warning('This is slow!')
        ...
        function doComposite(obj, ptr_dest, drawField, schematicFocus)
            
            % schematicField is the field in the layers collection that is
            % either empty, 'main', or 'preview'. When PreviewDrawMode is
            % 'schematic', we cannot draw the same texture to both the main
            % and preview windows. 'schematicField' tells Task Engine which
            % window to choose if it can't do both. We also pass
            % 'schematicFocus' ['main'/'preview'] to this method, which is 
            % indicates whether we are compositing to the main or preview
            % window. Note that if 'schematicField' is set,
            % 'schematicFocus' cannot be empty, as we would have nothing to
            % compare to. 
            if nargin <= 3
                schematicFocus = [];
            end
            ...
            switch drawField
                case 'drawOnWindow'
                    idx_draw = logical(cell2mat(layers(:, 4)));
                case 'drawOnPreview'
                    idx_draw = logical(cell2mat(layers(:, 5)));
            end
            if isempty(layers), return, end
            
            layers = layers(idx_draw, :);
            pos = layers(:, 7);
            sChoice = layers(:, 8);
            names = obj.prLayers.Names;
            names = names(idx_draw);
            layers = [cell2mat(layers(:, 1:3)), cell2mat(layers(:, 4:5)), cell2mat(layers(:, 6))];

            rect_dest = Screen('Rect', ptr_dest);

            numLayers = size(layers, 1);
            for l = 1:numLayers
                
                % if using schematic preview drawing, we can't draw to both
                % the main window and the preview. If a 'schematicField'
                % option has been passed, compare this to 'schematicFocus'
                % to determine whether we should draw this layer or not
                if isequal(obj.PreviewDrawMode, 'schematic') &&...
                    ~isequal(sChoice{l}, schematicFocus)
                    continue
                end
                
                rect_src = [0, 0, layers(l, 2), layers(l, 3)];
                if ~isequal(rect_src, rect_dest)
                    rect = teRectFromPreset(rect_src, rect_dest, pos{l});
                else
                    rect = [];
                end
                Screen('DrawTexture', ptr_dest, layers(l, 1), rect_src, rect);        
%                 teEcho('Drew layer %s to %s\n', names{l}, sChoice{l});
            end
            
        end
        
        function SummariseLayers(obj)
%             if isempty(obj.prLayers), return, end
%             items = obj.prLayers.Items;
%             items = cellfun(@struct, items, 'UniformOutput', false);
%             s = horzcat(items{:});
%             tab = struct2table(struct(s));
%             tab.layer = obj.prLayers.Keys';
%             tab = movevars(tab, 'layer', 'before', 1);
%             disp(tab)
            ...
            doDraw(l_main, obj.prWindowPtr)
            doDraw(l_preview, obj.prPreviewPtr)

            function doDraw(layers, ptr)
    ...
    methods (Hidden)
        ...
        % eye tracking
            ...
            % calib point sounds
            [file_calib, file_names] = findfiles(...
                fullfile(obj.CONST_PATH_ET, type,...
                'et_calib_snd*.wav'));
            for f = 1:length(file_calib)
                obj.Stim(file_names{f}) = teStim(file_calib{f});
            end            
        ...
        function prepareInternalStim(obj)
            ...
            % prepare internal stim in the normal way, which will create
            % separate textures for the preview window
                ...
                % where hard-coded texture pointers are used for future
                % drawing to these stim, select either the main window (for
                % composite preview drawing) or the preview window (for
                % schematic drawing) pointes
                if obj.prDoComposite
                ...
                elseif obj.prDoSchematic
                    obj.prTexture_ETFaceBoth = obj.Stim('et_face_both').PreviewTexturePtr;
                    obj.prTexture_ETFaceNone = obj.Stim('et_face_none').PreviewTexturePtr;
                    obj.prTexture_ETFaceLeft = obj.Stim('et_face_left').PreviewTexturePtr;
                    obj.prTexture_ETFaceRight = obj.Stim('et_face_right').PreviewTexturePtr;
                    obj.prTexture_ETFaceOutline = obj.Stim('et_face_outline').PreviewTexturePtr;
                end
...
%                 [], windowOption, [0, 0, obj.Resolution]);    
%             % turn on alpha blending 
%             Screen('BlendFunction', obj.prCameraWindowPtr, obj.CONST_ALPHA_BLEND_FACTOR_SRC,...
%                 obj.CONST_ALPHA_BLEND_FACTOR_DEST); 
%             % draw BG colour
%             Screen('FillRect', obj.ptr, obj.prBackColour);
