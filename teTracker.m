classdef teTracker < dynamicprops
    ...
    methods
        ...
        % parse inputs
            ...
            addParameter(   parser, 'message',          '',     @ischar     )
        ...
        % store
            ...
            % store variable info 
            obj.prVariables(end + 1, 1:6) =...
                {name, makeSubFolders, level, options, valFun, msg};
            % add dynamic prop
...
%                     trackers{t} = [];
        ...
        function InputValues(obj)
            ...
            for p = 1:length(pnames)
                ...
                % if set, get the message that we display to the user
                % before asking for the value. This is to add additional
                ...
                while ~happy
                    ...
                    % attempt to set the value. This will call the dynSet
                    % function, which in turn will attempt to validate the
                    % input, including by calling any specified validation
                    % functions
                    try
                        happy = true;
                        obj.(pnames{p}) = val;
...
%         function [suc, msg] = validateOneInput(obj, val, propName, valFun, options)
%             
%             % empty defaults for valFun and option if not passed to this
%             % function
%             if ~exist('valFun', 'var') 
%                 valFun = [];
%             end
...
%             end
%             
%             % try setting the value. This will call the dynset function for
%             % the property which will trigger validation. Catch any errors
%             % if validation fails
%             
%             
% %             % find index of the variable being set in the variables
% %             % definition property
% %             
% % 
% % 
...
function f = dynSet(~, propname)
        ...
        % this function is called whenever a dynamic property value is set.
        % This may be called by the InputValues method, or it could be set
        % by a simple set method (e.g. tracker.ID = 'A1') called from
        % elsewhere. Regardless of the source, the value needs to be
        % validated, and this happens here. 
        
            % find variable metadata (validation function, valid options
            % list etc) for this variable
            idx = strcmpi(obj.prVariables(:, 1), propname);
            if ~any(idx)
                error('Not a valid property name: %s.', propname)
            end
            valFun = obj.prVariables{idx, 5};
            options = obj.prVariables{idx, 4};
            
            % check that the value being assigned is char of numeric -
            % nothing else is supported
            if ~ischar(val) && ~isnumeric(val)
                error('Variable values must be either text or numeric');
            end
            ...
            if ~isempty(valFun)
                ...
                if ~tmpSuccess
                    if isempty(msg)
                        msg = sprintf('Invalid value entered for variable %s.',...
                            propName);
                    end
                    error(msg)
                end
            ...
            elseif ~isempty(options) 
                ...
                if ~optionValid
                    ...
                    error('Invalid value entered for %s.\n',...
                        propname);
            ...
            % all validation has passed, finally set the property value
            obj.(propname) = val;
            obj.GeneratePaths
