function [sync, tracker] = teSyncEEG_fieldtrip(tracker, data_ft, varargin)
...
% setup
    ...
    % check that, after excluding ignored events, we still have some events
    % left
    tmp = struct2table(ft_events);
    idx = ismember(tmp.value, eeg_code_ignored);
    if all(idx)
        sync.outcome = 'After excluding ignored events, the fieldtrip structure contained no events.';
        return
    end
...
% matched events are returned, or b) no sync marker is found in either EEG
% or TE data, we then look for other sync markers to use. Each time a sync
% marker is tested, markers are paired and tested

    suc_pair = false;
    while ~suc_pair

        % pair
        [suc_pair_marker, oc_pair, fail_code, pairs, numPairs, pairs_off] =...
            findAndPairSyncMarkers(syncMarker, tracker, codes_eeg, time_eeg, lab_te, time_te);
        
        % test if pairing was successful
        if suc_pair_marker
        ...
        % did we succeed?
        suc_pair = suc_pair_marker && all(numMatched > 1);
    ...
    % if we didn't successfully pair with SYNC marker and also didn't find
    % any alternative markers, give up
    if ~suc_pair && ~suc_search
        sync.outcome = 'SYNC markers either could not be found, or returned <2 matched events.';
        return
    end
...
% if <2 events were matched, look for alternative sync markers
    ...
    if all(numMatched <= 1)
        ...
        [suc_pair, oc_pair, fail_code, pairs, numPairs, pairs_off] = findAndPairSyncMarkers(...
...
% assess fit of each sync pair
    ...
    if sum(best_numMatched) > 1
        ...
        % if STILL more than one pair matches (e.g. if they were
        % identical), choose the first
        if sum(best_idx) > 1
            best_idx = best_idx(1);
        end
