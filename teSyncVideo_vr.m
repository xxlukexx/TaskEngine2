function ext = teSyncVideo_vr(inputVar)
% synchronise a screen recording video to a task engine session. The video
% is visually searched for corner markers and a QR code that contains
% session information. 
%
% Input is either a teExternalData_ScreenRecording object, or a path to a
% video file. If the input is a path, an teExternalData_ScreenRecording
...
% handle inputs
    ...
    if iscell(inputVar)
        ext = cellfun(@teSyncVideo_vr, inputVar, 'UniformOutput', false);
    end

    % check input format
    if ischar(inputVar) && exist(inputVar, 'file')
        
        % try to load the supplied path to an ext object
        try
            ext = teExternalData_ScreenRecording(inputVar);
            ext.Paths('screenrecording') = inputVar;
        catch ERR
            error('Error reading video file at %s\n\nError was:\n\n%s',...
                inputVar, ERR.Message)
        end
        
    elseif isa(inputVar, 'teExternalData_ScreenRecording')
        ext = inputVar;
    else
        error('Input must be either a teExternalData_ScreenRecording object, or a path to a video file.') 
    end
    
% setup zxing for decoding QR code
...
% process
    ...
    ext = syncOneVideo(ext);

    % write sync struct
    [sr_path, sr_file, sr_ext] = fileparts(ext.Paths('screenrecording'));
    file_out = fullfile(sr_path, sprintf('%s%s#%s#.sync.mat',...
        sr_file, sr_ext, ext.Sync.GUID{1}));
    sync = ext.Sync;
    save(file_out, 'sync')  
...
function ext = syncOneVideo(ext)

    file_in = ext.Paths('screenrecording');
...
% now loop through the search space and look for markers
    ...
    for f = 1:numFrames
        ...
        vr.CurrentTime = t;
%         fprintf('Time: %.2f\n', t);
        markerPresent(f) = findEdgeMarkers2(vr.readFrame);
        if markerPresent(f)
...
% number of frames, making for the fastest possible search. 
    ...
    for mrk = 1:numMarkers
        ...
        while abs(step) > (ifi / 2) 
            ...
            vr.CurrentTime = t_search;
            [markersFound, markers(mrk).details] = findEdgeMarkers2(vr.readFrame);
            if ~markersFound
        ...
        % decode QR
        vr.CurrentTime = markers(mrk).time_video;
        msg = decode_qr(imresize(vr.readFrame, 2));
        
        % if we could not decode the QR, try OCR instead
        if isempty(msg)
            vr.CurrentTime = markers(mrk).time_video;
            tmp = ocr(vr.readFrame);
            if ~isempty(tmp.Text)
    ...
    ext.Ext2Te = sync.video2te;
    ext.Te2Ext = sync.te2video;    
